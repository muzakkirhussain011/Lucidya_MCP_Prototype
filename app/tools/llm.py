...
def _deglue_text(s: str) -> str:
    if not s: return s
    s = _PAREN_OPEN.sub(r"\1 (", s)
    s = _PAREN_CLOSE.sub(r") \1", s)
...
def _clean(t: str) -> str:
    """Remove think blocks and clean whitespace."""
...
def ollama_generate(prompt: str, system: str = "", temperature: float = 0.3, max_tokens: int = 512) -> str:
...
def _chunk_markdown(buffer: str) -> tuple[list[str], str]:
    ...
    while i < len(buffer):
        ...
        if c == "\n":
            ...
            chunks.append(buffer[last:i])
            last = i
        
        if flush:
            chunks.append(buffer[last:i+1])
            last = i + 1
        i += 1
...
def ollama_generate_markdown_stream(prompt: str, system: str = "", temperature: float = 0.3) -> Iterator[str]:
    """
    Streams markdown-friendly chunks from Ollama with de-gluing.
    Properly yields content even when model produces minimal output.
    """
    s = get_settings()
    ...
    try:
        with requests.post(url, json=payload, stream=True, timeout=OLLAMA_TIMEOUT_SECONDS) as r:
            ...
            for line in r.iter_lines(decode_unicode=True):
                ...
                # If this is the final chunk or buffer is getting large, process it
                if data.get("done", False) or len(buf) > 100:
        ...
        if total == 0:
            log.warning("LLM stream produced no content, generating fallback")
            fallback = "- Analysis in progress. Please check if the model is loaded correctly.\n"
            yield fallback
            total = len(fallback)
        ...
        yield f"_Streaming error: {str(e)}_"